...
ASSET_DIR=$TESTCASE4_HOME/img_thumb/assets
cd $TESTCASE4_HOME/img_thumb/src
...
image=$ASSET_DIR/small.jpg
if [ ! -f $image ]; then
    echo "image $image does not exist, quit."
    exit
fi
image=$ASSET_DIR/medium.jpg
if [ ! -f $image ]; then
    echo "image $image does not exist, quit."
    exit
fi
image=$ASSET_DIR/large.jpg
if [ ! -f $image ]; then
    echo "image $image does not exist, quit."
    exit
fi
java -cp upload-image/target/upload-image.jar org.serverlessbench.UploadImage $image small.jpg $couchdb_url $COUCHDB_USERNAME $COUCHDB_PASSWORD $IMAGE_DATABASE
java -cp upload-image/target/upload-image.jar org.serverlessbench.UploadImage $image medium.jpg $couchdb_url $COUCHDB_USERNAME $COUCHDB_PASSWORD $IMAGE_DATABASE
java -cp upload-image/target/upload-image.jar org.serverlessbench.UploadImage $image large.jpg $couchdb_url $COUCHDB_USERNAME $COUCHDB_PASSWORD $IMAGE_DATABASE
